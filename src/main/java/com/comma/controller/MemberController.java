...
//        }
    ...
    @ResponseBody
    public HashMap<String,Object> saveMembersSave(@RequestBody List<Member> member) {
        HashMap<String,Object> response = new HashMap<>();
        try {
            memberService.saveAll(member);

            response.put("status", true);
            response.put("message", "멤버가 업데이트가 완료되었습니다.");
        } catch (Exception e) {
            response.put("status", false);
            response.put("message", "멤버가 업데이트에 실패했습니다.");
        }
